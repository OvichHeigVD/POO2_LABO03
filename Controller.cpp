...
void Controller::display() const {
    ...
    if(this->boat->getAnchoredBank() == this->leftBank){
        std::cout << *this->boat << std::endl;
    }else{
        std::cout << std::endl;
    }
    std::cout << Controller::riverSeparator() << std::endl;
    if(this->boat->getAnchoredBank() == this->rightBank){
        std::cout << *this->boat << std::endl;
    }else{
        std::cout << std::endl;
    }
...
bool Controller::controlAccompanimentRulesOnBank(Container* where) {
    ...
    for(const Person* person : *where){
        if(person->getType() == Person::Family) hasFamilyMember = true;
        if(person->getType() == Person::Police) hasPoliceman = true;
        if(person->getType() == Person::Thief)  hasThief = true;
        if(person->isChild()){
...
{
    const char* chars = " \t\v\r\n";
    return str.erase(0, str.find_first_not_of(chars)).erase(str.find_last_not_of(chars) + 1);
}
